...
enum class TILEBOX_EXPORT X11InputEventMask : std::uint32_t
{
    X11NoEventMask = 0,
    X11KeyPressMask = 1 << 0,
...
TILEBOX_EXPORT [[nodiscard]] auto EventMaskFromXlibEventMask(const std::int64_t event_mask) -> X11InputEventMask;

/// @brief Converts a tilebox input event mask to a Xlib input event mask
TILEBOX_EXPORT [[nodiscard]] auto EventMaskToXlibEventMask(const X11InputEventMask event_mask) -> std::int64_t;
...
enum class TILEBOX_EXPORT X11EventType : std::uint8_t
{
    X11KeyPress = 2,
    X11KeyRelease = 3,
    X11ButtonPress = 4,
...
TILEBOX_EXPORT [[nodiscard]] auto EventFromXlibEvent(std::int32_t event_type) noexcept -> X11EventType;

TILEBOX_EXPORT [[nodiscard]] auto EventToXlibEvent(X11EventType event_type) noexcept -> std::int32_t;
