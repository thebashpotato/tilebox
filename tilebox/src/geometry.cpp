...
auto Rect::GetX() const noexcept -> std::int32_t
{
    return point.x.value;
}

auto Rect::GetY() const noexcept -> std::int32_t
{
    return point.y.value;
}

auto Rect::GetW() const noexcept -> std::uint32_t
{
    return width.value;
}

auto Rect::GetH() const noexcept -> std::uint32_t
{
    return height.value;
}
...
auto Rect::ShrinkIn(const uint32_t border) const noexcept -> Rect
{
    Width w;
    Height h;
...
auto Rect::ScaleWidth(const double_t factor) const noexcept -> Rect
{
    if (factor == 1.0)
    {
        return *this;
...
auto Rect::ScaleHeight(const double_t factor) const noexcept -> Rect
{
    if (factor == 1.0)
    {
        return *this;
...
auto Rect::ContainsPoint(const Point &other) const noexcept -> bool
{
