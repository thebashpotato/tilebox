...
X11Window::~X11Window()
{
    ...
    if (_id != 0 && _dpy.is_connected())
    {
        ...
        XDestroyWindow(_dpy.get_raw(), _id);
        _id = 0;
    }
}
...
auto X11Window::create_window(const Rect &r) noexcept -> bool
{
    ...
    wa.background_pixel = BlackPixel(_dpy.get_raw(), _dpy.screen_id());
    wa.border_pixel = WhitePixel(_dpy.get_raw(), _dpy.screen_id());
    wa.event_mask = ButtonPress;

    if (!_is_mapped)
    {
        _id = XCreateWindow(_dpy.get_raw(), _dpy.root_window(), r.x(), r.y(), r.w(), r.h(), 0,
                            DefaultDepth(_dpy.get_raw(), _dpy.screen_id()), InputOutput,
                            DefaultVisual(_dpy.get_raw(), _dpy.screen_id()), CWBackPixel | CWBorderPixel | CWEventMask,
                            &wa);
...
auto X11Window::map() noexcept -> bool
{
    if (!_is_mapped)
    {
        if (XMapWindow(_dpy.get_raw(), _id) == BadWindow)
        {
            return _is_mapped;
        }
        _is_mapped = true;
